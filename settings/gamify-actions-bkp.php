...
function generate_action_fields() {
	...
	foreach ( $options as $action_id => $field_array ) {
		if ( ( 'daily_limit' !== $action_id ) && ( 'daily_limit_activate' !== $action_id ) ) {
			ob_start();

			$settings_value =(isset( $options[$action_id]['delete'] ) ? $options[$action_id]['delete'] : 'unchecked');
			echo "<th scope='row' class='check-column'><input type='checkbox' id='gamwp_action_settings[" . $action_id . "][delete]' name='gamwp_action_settings[" . $action_id . "][delete]' value='checked' " . checked( 'checked', isset( $settings_value ) ? $settings_value : 'unchecked', false ) . " /></td>";

			$settings_value =(isset( $options[$action_id]['activity_title'] ) ? $options[$action_id]['activity_title'] : '');
			echo "<td class='action-title column-action-title'><input type='text' id='gamwp_action_settings[" . $action_id . "][activity_title]' name='gamwp_action_settings[" . $action_id . "][activity_title]' value='" . esc_html( $settings_value ) . "' title='" . esc_html( $settings_value ) . "'placeholder='Action Title' /></td>";
	...
	<div id="gamwp-settings-wrap" class="wrap">
		...
		<p><a href="">WIKI LINK</a>.</p>
			...
			<table class="wp-list-table widefat fixed posts" id="gamwp-action-table">
				<thead>
					<tr>
						<th scope="col" class="manage-column check-column"><?php _e('Del', 'gamwp'); ?></th>
						<th scope="col" id="action-title" class="manage-column column-action-title sortable desc">
							<a href="admin.php?page=gamify-actions.php&orderby=action-title&order=asc">
								<span><?php _e('Action Title', 'gamwp'); ?></span>
								<span class="sorting-indicator"></span>
							</a>
						</th>
						<th scope="col"><?php _e('Action Hook', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Points', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Daily Limit', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Use Once', 'gamwp'); ?></th>
					</tr>
				</thead>
				<tfoot>
					<tr>
						<th scope="col" class="manage-column check-column"><?php _e('Del', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Action Title', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Action Hook', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Points', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Daily Limit', 'gamwp'); ?></th>
						<th scope="col"><?php _e('Use Once', 'gamwp'); ?></th>
